...
void Client::_handle_response(uint16_t code, const std::vector<uint8_t> &p)
{
    ...
    case RES_MESSAGES_LIST: {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            /* receiver_uuid */    read_fixed(p, off, 36);
            int64_t     ts       = read_i64(p, off);
            std::string body     = read_fixed(p, off, 512);
            client_private_message_print_messages(
                sender.c_str(), static_cast<time_t>(ts), body.c_str());
        }
        break;
    }
    ...
    case ERR_INVALID_COMMAND:
        std::cerr << "Server rejected the command (invalid)." << std::endl;
        break;
    ...
    default:
        std::cerr << "Unknown server code: 0x"
